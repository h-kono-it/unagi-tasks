...
| `ALLOWED_GITHUB_ID` | 許可する GitHub 数値ID（`curl https://api.github.com/users/{username} \| grep '"id"'` で確認） |

## デプロイ

Deno Deploy の GitHub integration で **Fresh プリセット**を選択するとビルド・デプロイが自動で行われる。

環境変数は Deno Deploy の Settings > Environment Variables に設定する。

デプロイ後、GitHub OAuth App の callback URL を `https://{project}.deno.dev/auth/callback` に更新する。

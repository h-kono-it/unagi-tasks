...
GitHub OAuth で認証し、GitHub アカウントを持つ誰でも利用できる。

#### フロー

```
ユーザー → /auth/signin（ランディング） → /auth/github → GitHub認証画面 → /auth/callback
→ GitHubユーザー情報取得 → Deno KVにセッション保存 → トップページへ
```
...
routes/
  auth/
    signin.tsx    # ランディングページ（サインインボタン）
    github.ts     # GitHubへリダイレクト
    callback.ts   # コールバック処理 + セッション発行
    signout.ts    # セッション削除
  _middleware.ts  # 全ルートの認証チェック（公開パスは除外）
utils/
...
Denoの**Freshフレームワーク**ですね。Preactベースのアイランドアーキテクチャによる高速な描画と、**Deno
KV**のシームレスな統合は、今回の「爆速で判断・入力する」という目的に非常にマッチしています。
...
- **Next Buttons:**
  「完了（Done）」「今は無理（Skip）」「削除（Delete）」の3ボタンのみ。
- **Hotkeys:** `Enter`で完了、`S`でスキップなど。

### B. Quick Inbox (割り込み用)

- PCの端に常に置いておく、または特定のショートカットでフォーカスが当たる入力欄。
- **No Selection:** カテゴリや期限は選ばせない。「入力してEnter」だけで
  `["inbox", ...]` に保存し、即座に元の作業に戻れるようにします。
...
3. **Phase 3: スコアリング＆表示**
   - `inbox` 内のアイテムを `calculateScore`
     でソートし、トップ1件をメイン画面に出す。
4. **Phase 4: 割り込み整理フェーズ**
   - 一定時間ごとに「整理モード」に切り替わり、溜まった `inbox` に属性（Energy,
     Priority）を付与して `tasks` に移動させるUI。

---

### 最初の実装ポイント

まずは **「Deno KV にタスクを投げ込み、それをリストで出す」**
という一番シンプルな部分からコードに落としていきましょうか。
